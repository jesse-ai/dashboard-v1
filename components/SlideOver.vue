<template>
    <TransitionRoot as="template" :show="(model as boolean)">
        <Dialog as="div" static class="fixed inset-0 overflow-hidden z-40" :open="(model as boolean)" @close="model = false">
            <div class="absolute inset-0 overflow-hidden">
                <TransitionChild as="template" enter="ease-in-out duration-400" enter-from="opacity-0" enter-to="opacity-100" leave="ease-in-out duration-400" leave-from="opacity-100" leave-to="opacity-0">
                    <DialogOverlay class="absolute inset-0 bg-gray-500 bg-opacity-95 transition-opacity" />
                </TransitionChild>

                <div class="fixed inset-y-0 right-0 pl-10 max-w-full flex">
                    <TransitionChild as="template" enter="transform transition ease-in-out duration-400 sm:duration-400" enter-from="translate-x-full" enter-to="translate-x-0" leave="transform transition ease-in-out duration-400 sm:duration-400" leave-from="translate-x-0" leave-to="translate-x-full">
