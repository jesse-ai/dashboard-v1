<template>
    <div class="w-full">
        ...
        <UForm :state="form" class="space-y-4" @submit="submit">
            ...
            <UFormGroup label="Email (must be registered with on Jesse.Trade)" description="Enter your email address for us to know who sent the feedback and possibly reply back to you. It must be the email address of your account on Jesse.Trade" required>
                <UInput v-model="form.email" placeholder="Email address..." type="email" />
            </UFormGroup>

            <div class="flex justify-end">
                <UButton id="feedback-cancel-button" color="gray" variant="link" class="mr-8" label="Cancel" @click="close()" />
