<template>
  ...
  <!-- Logout -->
  <ConfirmModal title="Logout" description="Are you sure you want to logout?" type="info" v-model="logoutConfirmation">
    <UButton @click="logout" variant="solid" color="rose" size="lg" label="Logout" />
  </ConfirmModal>
              ...
              <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                <MenuItems class="bg-white dark:bg-gray-700 origin-top-right absolute right-0 mt-2 w-64 rounded-md border-gray-200 dark:border-gray-900 shadow-lg py-1 ring-1 ring-black ring-opacity-5 focus:outline-none divide-y divide-gray-100 dark:divide-gray-900">
                  <div class="px-4 py-3 select-text">
                    <p class="text-sm text-gray-500 dark:text-gray-300">Active License: <span class="truncate text-sm font-medium text-gray-800 dark:text-gray-200 opacity-100 uppercase">{{
                      planInfo.plan
                        }}</span></p>
                  </div>
