            ...
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center">
                    ...
                    <div class="hidden lg:block lg:ml-6">
                        <div class="flex space-x-4">
                            <router-link v-for="item in navigation" :id="convertToSlug(item.name) + '-page-button'" :key="item.name" :to="item.to" class="flex items-center text-gray-700 dark:text-gray-300 px-3 py-2 rounded-md text-sm font-medium" :class="($route.path.startsWith(item.to) && item.to !== '/') || $route.path === item.to ? 'bg-gray-200 dark:bg-gray-900' : 'hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-600 dark:hover:text-gray-200'">
                ...
                <div class="hidden lg:ml-6 lg:block">
                    <div class="flex items-center">
                        ...
                        <button data-cy="settings-icon" class="btn-nav" @click="modals.settings = true">
                            ...
                            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                                <MenuItems data-cy="nav-dropdown-menu-items" class="bg-white dark:bg-gray-700 origin-top-right absolute right-0 mt-2 w-64 rounded-md border-gray-200 dark:border-gray-900 shadow-lg py-1 ring-1 ring-black ring-opacity-5 focus:outline-none divide-y divide-gray-100 dark:divide-gray-900">
                                    ...
                                    <div class="py-1">
                                        ...
                                        <a href="https://jesse.trade/pricing" :class="[active ? 'bg-gray-100 dark:bg-gray-800' : '', 'flex justify-start items-center w-full text-left px-4 py-2 text-sm text-yellow-500 dark:text-yellow-300']" target="_blank">
                                        ...
                                        <button data-cy="nav-create-strategy" :class="[active ? 'bg-gray-100 dark:bg-gray-800' : '', 'flex justify-left items-center w-full text-left px-4 py-2 text-sm text-indigo-700 dark:text-indigo-300']" @click="modals.makeStrategy = true">
                                    ...
                                    <div class="py-1">
                                        <MenuItem v-slot="{ active }">
                                        <button data-cy="nav-create-strategy" :class="[active ? 'bg-gray-100 dark:bg-gray-800' : '', 'flex justify-left items-center w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300']" @click="openFeedback">
                                        ...
                                        <MenuItem v-slot="{ active }">
                                        <button data-cy="nav-help-center-link" href="https://jesse.trade/help" :class="[active ? 'bg-gray-100 dark:bg-gray-800' : '', 'flex justify-start items-center w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300']" target="_blank" @click="modals.about = true">
        ...
        <DisclosurePanel class="lg:hidden">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <div v-for="item in navigation" :id="convertToSlug(item.name) + '-page-button'" :key="item.name" :class="[$route.path.startsWith(item.to) ? 'bg-gray-200 dark:bg-gray-900' : 'hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-600 dark:hover:text-gray-200', 'rounded-md']">
                    <router-link :to="item.to" class="text-gray-700 dark:text-gray-300 text-sm font-medium">
            ...
            <div class="px-2 py-2 space-y-1 border-t border-gray-200 dark:border-gray-900">
                ...
                <button class="flex justify-start items-center w-full text-left px-2 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-600 dark:hover:text-gray-200 rounded-md" @click="modals.makeStrategy = true">
                    <DocumentAddIcon class="w-5 h-5 mr-2" />
                    New Strategy
                </button>
            </div>
            ...
            <div class="px-2 py-2 space-y-1 border-t border-gray-200 dark:border-gray-900">
                ...
                <div class="w-full">
                    <button href="https://jesse.trade/strategies" class="flex justify-start items-start w-full text-left px-2 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-600 dark:hover:text-gray-200 rounded-md" target="_blank" @click="modals.makeStrategy = true">
                        <IdentificationIcon class="w-5 h-5 mr-2" />
                        About
                    </button>
                </div>
            ...
            <div class="flex justify-between items-center px-2 py-2 space-y-1 border-t border-gray-200 dark:border-gray-900">
                <button class="btn-nav ml-0" @click="modals.settings = true">
                    <span class="sr-only">Settings</span>
                    <CogIcon class="h-6 w-6" aria-hidden="true" />
                </button>
...
export default {
    ...
    computed: {
        ...mapState(useAuthStore, ['modals', 'hasLivePluginInstalled', 'planInfo']),
