  ...
  <div class="mb-4">
    <div class="hidden sm:block">
      <nav class="relative rounded-lg shadow flex divide-x divide-gray-200 dark:divide-gray-700"
        ...
        <div v-for="(tab, name, index) in props.tabs" :key="tab.id" :data-cy="'tab' + index"
             class="relative group min-w-0 flex-1 overflow-hidden text-center flex items-center ">
          <NuxtLink :to="`/candles/${tab.id}`"
          ...
          <!-- Tab close button -->
          <button v-show="Object.keys(props.tabs).length > 1" :data-cy="'tab-close-button' + index"
